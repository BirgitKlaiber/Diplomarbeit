...
\begin{center}
\textcolor{blue}{Version 20180722}
...
\noindent
Äquivalenzklasse 1: $  \{(contact(bob,bon) * cold(bob))[0.0], (contact(anna,anna) * cold(anna)[0.0], \\ (contact(carl,carl) * cold(carl))[0.0] \} $ \\
Äquivalenzklasse 2: $ \{(contact(bob,carl) * cold(carl))[0.02068], \\(contact(carl,bob) * cold(bob))[0.02068] \}$\\
Äquivalenzklasse 3:  $ \{(contact(carl,anna) * cold(anna))[0.03588],\\ (contact(anna, carl) * cold(carl))[0.03588], (contact(anna,bob) * cold(bob))[0.03588],\\ (contact(bob,anna) * cold(anna))[0.03588]\}$

\noindent
Generalisiertes Konditional für Äquivalenzklasse 1:\\ $ \{ (contact(X,Y) * cold(Y))[0.0]<X=Y>\} $\\
Generalisiertes Konditional für Äquivalenzklasse 2:\\ $ \{ (contact(X,Y) * cold(Y))[0.02068]<(X=bob * Y=carl + X=carl * Y=bob)>\} $\\
Generalisiertes Konditional für Äquivalenzklasse 3: \\$ \{ (contact(X,Y) * cold(Y))[0.03588]<((((X=carl * Y=anna) + X=anna* Y=carl) + X=anna * Y=bob) + X=bob * Y=anna) >\} $\\
...
\begin{Bsp}[Negative Generalisierung am Beispiel ColdSpez]
	...
	\noindent
		Generalisiertes Konditional für Äquivalenzklasse 1:\\ $ \{ (contact(X,Y) * cold(Y))[0.0]<X=Y>\} $\\
	Generalisiertes Konditional für Äquivalenzklasse 2:\\ $ \{ (contact(X,Y) * cold(Y))[0.02068]<(X!=anna * Y!=anna * X!=Y)>\} $\\
		Generalisiertes Konditional für Äquivalenzklasse 3: \\$ \{ (contact(X,Y) * cold(Y))[0.03588]<(X=anna + Y=anna) * X!=Y>\} $\\
...
\includegraphics[scale = 0.7]{Graphics/Lo}
\begin{figure}[h]
...
Das ist der trivialste Fall. In diesem Fall besteht die Rückgabemenge aus dem quantifizierten Konditional ohne Constraint.
	\begin{Bsp}[Konditionale Generalisierung am Beispiel "{}BirdsWithoutNull"{}]
	S. \ref{BirdsWihoutNull}\\
...
\begin{itemize}
	...
	\begin{Bsp}[Konditionale Generalisierung am Beispiel Birds]
	S. \ref{Birds}.\\
	...
	\begin{itemize}
		...
		\begin{Bsp}[Konditionale Generalisierung bei zwei Klassen am Beispiel Cold]
			S. \ref{Cold}\\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{Bsp}[Konditionale Generalisierung am Beispiel ColdSpez]
			S. \ref{ColdSpez}\\
...
\includegraphics[scale = 0.8]{Graphics/Fehler_ColdSpez}
\begin{figure}[h]
...
\includegraphics[scale = 1.0]{Graphics/Aenderung_31}
\begin{figure}[h]
	\caption{Fraction.lcm -alt-}
\end{figure}


\includegraphics[scale = 1.0]{Graphics/Aenderung_32}
\begin{figure}[h]
