...
\textbf{Mögliche Fallunterscheidungen:}\label{Fallunterscheidung Formel MehrAequi}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item ein- und zweistelliges Prädikat: 
		\end{itemize}
		\item Alle Äquivalenzklassen enthalten mehr als ein Konditional
			\begin{itemize}
			\item einstelliges Prädikat: kein Beispiel gefunden.
...
\begin{itemize}
	...
	\item Logische Optimierung TODO
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item zweistelliges Prädikat: Bei zweistelligen Prädikaten wird es immer eine Klasse geben, die das reflexive Wissen enthält. Für diese Klasse bildet man den Constraint als Constraint der Ungleichheit für die beiden Variablen des zweistelligen Prädikats. An dieser Stelle wird es kompliziert. Die übrigen Klassen unterscheiden sich darin, ob sie spezifisches Wissen enthalten oder nicht. Da aber im Voraus keine Überprüfung der Konditionale mit der Wissensbasis erfolgt, muss man dies durch einen geschickten Vergleich der Konditionale erreichen TODO Anmerkung: vielleicht muss man doch mit der Wissensbasis vergleichen, das ist kein trivialer Vergleich...große Not...Der für die jeweilige Klasse erhaltene Constraint muss dann noch mit einem Contraint der Ungleichheit für reflexives Wissen konjunktiv verknüpft werden.  
			\end{itemize}
		\begin{Bsp}[Konditionale Generalisierung am Beispiel ColdSpez, \ref{ColdSpez}]
			...
			Klasse 1: $ {(contact(anna,ann) \land cold(anna)), (contact(bob,bob) \land cold(bob)), (contact(carl,carl) \land cold(carl)) } $\\
			Klasse 2: $ {(contact(bob,carl) \land cold(carl)), (contact(carl,bob) \land cold(bob)) } $\\
			Klasse 3: $ {(contact(carl,anna) \land cold(anna)), (contact(ann, carl) \land cold(carl)), (contact(anna,bob) \land cold(bob)), (contact(bob, anna) \land cold(anna)) } $\\
			\\
			\noindent
			Konditional für Klasse 1: $ {(contact(X,Y) \land cold(Y)) [0.0] <X = Y>} $\\
