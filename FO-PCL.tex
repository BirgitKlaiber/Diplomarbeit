...
\begin{center}
\textcolor{blue}{Version 20180211}
...
\begin{Def}[Spezifische Konstanten] \index{Konstante ! spezifisch} \label{Konstanten spezifisch}
	Sei $ \Sigma = (S, {\cal D}, Präd) $ eine mehrsortige Signatur und sei $ \cal{R}  $ eine FO-PCL Wissensbasis, gemäß Definition \ref{KB}. Dann gilt für alle Konstanten $ d \in \cal{D} $: $ d $ ist eine spezifische Konstante, wenn es ein probabilistisches Konditional $ R \in \cal{R}$  gibt, so dass $ d $ in der Konklusion oder der Prämisse vorkommt. 
\end{Def}
Ein Konditional der Wissensbasis ist genau dann ein spezifisches Konditional, wenn die die Prämisse oder Konklusion (oder auch beide) eine Konstante enthalten. Das bedeutet, dass die Wissensbasis über differenziertere Informationen über ein Objekt des Universums verfügt. Eine spezifische Konstante kommt somit in einem Konditional der Wissensbasis vor. 
}

\textcolor{blue}{
\begin{Def}[Generische Grundinstanzen von Konditionalen] \index{Grundinstanz ! generisch} \label{Grundinstanz generisch}
Sei $ \Sigma = (S, {\cal D}, Präd) $ eine mehrsortige Signatur und sei $ \cal{R}  $ eine FO-PCL Wissensbasis, gemäß Definition \ref{KB}. Sei $ R \in \cal R $ und $ t \in Term_{\Sigma}  $. Des Weiteren sei $  D_s $ die Menge der in der Wissensbasis vorkommenden spezifischen Konstanten. Sei $ gnd(R) $ die Menge der zulässigen Grundinstanzen von $ R $ gemäß Definition \ref{Grundinstanz}.
\\
\\
Die Menge $ genGnd(R) $ der zulässigen generischen Grundinstanzen von $ R $ ist definiert als eine Teilmenge der zulässigen Grundinstanzen $ genGend(R) \subset gnd(R) $, für die gilt:\\
...
\textcolor{blue}{
	\begin{Def}[Spezifische Grundinstanzen von Konditionalen] \index{Grundinstanz ! spezifisch} \label{Grundinstanz spezifisch}
		...
		\\
		Die Teilmenge der zulässigen Grundinstanzen von $ R $, die keine generischen Grundinstanzen sind:\\
		 $ spezGnd(R) = gnd(R) \bigcap genGnd(R)$.
	\end{Def}
...
Sei $ \Sigma = (S, {\cal D}, Präd) $ eine mehrsortige Signatur und sei $ \cal{R}  $ eine FO-PCL Wissensbasis, gemäß Definition \ref{KB}. Sei ferner $ \cal{Q} $ eine Anfrage in Form eines unbedingten Konditionals,  $ \cal{E}$  $ = \{E_1, ..., E_j\}$ eine Menge von Äquivalenzklassen gemäß Definition \ref{Äquivalenzklassen}.\\
	Eine Generalisierung ist eine Abbildung, die jeder Äquivalenzklasse $ E_i \in \cal{E} $  $ mit$  $ i \in \{1, ..., j\} $ jeweils genau ein einziges Konditional zuordnet. 
	Dieses Konditional wird auch generalisiertes Konditional genannt. \index{Konditional ! generalisiertes}
\end{Def}
...
\end{Def}
Dagegen ist unter einem quantifizierten Konditional ein Konditional mit zugeordnete Wahrscheinlichkeit zu verstehen.
\begin{Def}[Ausgabe der Komponente] \index{Ausgabe der Komponente} \label{Ausgabe der Komponente}
	Sei $ \Sigma = (S, {\cal D}, Präd) $ eine mehrsortige Signatur und sei $ \cal{R}  $ eine FO-PCL  Wissensbasis, gemäß Definition \ref{KB}. Sei ferner  $ \cal{Q} $ eine nichtleere Menge von Anfragen in Form qualitativer Konditionale, $ \cal G $ die Menge aller quantifizierten Konditionale.\\
	Die Ausgabe der Komponente ist das Bild der Abbildung:\\
		\hspace{3.0 cm} $ \delta $: $ \cal{Q}  $ $  \rightarrow  \cal{G}$, $ \delta(q)  $ = $ \{g \in G \mid $  g ist ein logisch minimales und generalisiertes Konditional $\} $\\
		die jeder Anfrage $ q \in \cal Q $ (in Form eines qualitativen Konditionals) eine nichtleere Menge $ G \subset \cal G $ von Konditionalen zuordnet. 
	\end{Def}}
...
Bei den Beispielen findet sich der Begriff "{}angestrebte Ausgabe"{}.\index{angestrebte Ausgabe}\\ \label{angestrebte Ausgabe}
\emph{Unter der angestrebten Ausgabe im Sinne dieser Arbeit ist das Ergebnis eines Prozesses der Betrachtung und des Vergleichs der Klassen und daraus resultierend der Generierung eines möglichst allgemeinen Konditionals zu verstehen. Sie ist das Ergebnis einer manuellen Betrachtung. Dabei wird angestrebt, dass der Constraint des erzeugten Konditionals möglichst kurz ist, d.h., er soll möglichst wenig Junktoren und Terme enthalten. Es ist das Ziel, im Wege des Inferenzprozesses eine Menge von Konditionalen zu erzeugen, die identisch oder zumindest äquivalent mit der angestrebten Ausgabe sind.  Die generierten Konditionale sollten dabei intersubjektiv als möglichst allgemein akzeptiert werden. Ob das algorithmisch so umsetzbar ist, werden die weiteren Forschungen ergeben. }
}
...
\begin{Def}[Logische Minimierung in FO-PCL] \label{Logische Min} \index{Logische Minimierung}
	Sei $\cal{R}$ eine FO-PCL Wissensbasis, wobei definitionsgemäß $\cal{R}$  = \{$ R_1, ..., R_m $\} eine Menge von FO-PCL Konditionalen ist. Sei $ R $ ein FO-PCL Konditional mit Constraint $ C $. Sei $ \cal B $ die Menge der booleschen Gesetze.\\
	Ein FO-PCL Konditional ist genau dann logisch minimiert, wenn der Constraint $ C $ logisch minimiert ist. Der Constraint $ C $ ist genau dann logisch minimiert, wenn gilt: es gibt kein Gesetz $ B $ aus der Menge der booleschen Gesetzte $ \cal B $, das anwendbar wäre und dessen Anwendung zu einer Verkleinerung der Summe der in $ C $ vorkommenden Junktoren und Terme führen würde.
\end{Def}
...
\begin{footnotesize}
	\begin{longtable}{ | p {0.5\currentLongTableWidth} | p {0.5\currentLongTableWidth}  |}
		...
		\endlastfoot
		\hline
		Definitionen sollten überarbeitet werden (insbesondere Eingangssatz); dazu gehören: \newline angestrebte Ausgabe \newline Generalisierung \newline Äquivalenzklassen \newline spezifische Konstanten \newline generische Grundinstanzen halbverbal definieren \newline spezifische Grundinstanzen halbverbal definieren \newline Ausgabe der Komponente
		...
		Neue Definition: Qualitatives Konditional
		& \ref{qualitatives_Konditional}\\
		\hline
		logische Minimierung verbal definieren
