...
\begin{center}
\textcolor{blue}{Version 20180109}
...
\begin{Def}[Ausgabe der Komponente] \index{Ausgabe der Komponente} \label{Ausgabe der Komponente}
	Sei $\cal{R}$ eine FO-PCL Wissensbasis, wobei definitionsgemäß $\cal{R}$  = $ {R_1, ..., R_m} $ eine Menge von FO-PCL Konditionalen ist, $ \cal{Q} $ eine Anfrage in Form eines unbedingten Konditionals.\\
	Die Ausgabe der Komponente ist das Bild der Abbildung:\\
		\hspace{3.0 cm} $ \delta $: $ \cal{Q}  $ $  \rightarrow  \cal{R}$  = $\{R_1, ..., R_j\}   $,\\
		die jedem unbedingten Konditional eine nichtleere Menge von bedingten Konditionalen zuordnet. 
	\end{Def}
}
...
\includegraphics[scale = 0.8]{Graphics/Flussplan_Generalisierung_nichtatomar}
\begin{figure}[h]
...
\includegraphics[scale = 0.6]{Graphics/Flussplan_Generalisierung_zweiKlassen}
\begin{figure}[h]
...
\begin{footnotesize}
	\begin{longtable}{ | p {0.5\currentLongTableWidth} | p {0.5\currentLongTableWidth}  |}
		...
		& sh. \ref{Logische Genopt} und \ref{Minimierung} \\
		\hline
		Idee: Flussplan fertig machen und ins Dokument einbauen
		& \ref{Fig_General}, \ref{Fig_Log_Genopt}, \ref{Fig_ZweiKlassen} \\
		\hline
