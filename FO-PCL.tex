...
\chapter{Experimentelle Forschungen}
...
\subsection{Merkmal der Gleichheit}
...
\subsubsection {Eine Äquivalenzklassen und die Anfrage ist ein Atom} \label{Atom_eineKlasse}
...
\subsubsection {Eine Äquivalenzklassen und die Anfrage ist eine (nichtatomare, logische) Formel} TODO


\subsection{Zwei Äquivalenzklassen}

\subsubsection {Zwei Äquivalenzklassen und die Anfrage ist ein Atom}
\textcolor{blue}{
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item mehrstelliges Prädikat (Exemplarisch: $ likes(a,V)$ in "{}Misanthrope"{}.        
	\end{itemize}
...
\paragraph{ Mehr als zwei Äquivalenzklassen und die Anfrage ist ein Atom}
\begin{itemize}
	...
	\item Eine Äquivalenzklasse enthält ein Konditional, die anderen mehr als ein Konditional
	\begin{itemize}
		\item einstelliges Prädikat: dazu ließ sich kein Beispiel finden. Theoretisch könnte es sich ergeben, wenn mehrere spezifische Konstanten vorhanden sind (für die sich dann auch jeweils eine eigene Klasse ergibt) und eine Klasse mit den generischen Grundinstanzen.\\
		\item zweistelliges Prädikat:\\
		Der Fall kann nicht eintreten und muss daher nicht behandelt werden.  
	\end{itemize}
	...
	\begin{itemize}
		...
		\item mehrstelliges Prädikat (Beispiele hierfür sind die Anfragen $ contact(X,Y) $ in "{}ColdSpez"{} und "{}ColdTransSpez"{} und $ likes(U,V) $ in "{}Misanthrope"{} und "{}MisanthropeIrreflexive"{}) \\
	\end{itemize}
\end{itemize}
}
TODO:
\subsubsection {Zwei Äquivalenzklassen und die Anfrage ist eine (nichtatomare, logische) Formel}

\chapter{Ideen für Algorithmen}
\section{Vorüberlegungen}
...
\textcolor{blue}{
	...
	\begin{Bsp}[Erstrebte Ausgabe am Beispiel "{}Misanthrope"{}, vgl. auch \ref{sec:Misanthrop} und \ref{Misanthrope}]
		...
		Konditional für Klasse 3: $ \{(likes(U,V))[0.05] <U = a \lor V = a>\} $\\
	\end{Bsp}
}
...
\textcolor{blue}{Fazit zur positiven Generalisierung:\\
...
\begin{Bsp}[Negative Generalisierung am Beispiel "{}Cold"{}, s. \ref{Cold} ]:
	...
	Konditional für Klasse 2: $ \{(contact(X,Y))[0.486]<(((X neq anna \land Y neq anna) \land (X neq bob \land Y neq bob)) \land (X neq carl \land Y neq carl))>\} $\\
\end{Bsp}
}
\textcolor{blue}{Fazit zur negativen Generalisierung:\\
...
Die Experimente hierzu findet man unter \ref{Atom_eineKlasse}.\\
...
\end{itemize}
\item Beide Äquivalenzlassen enthalten mehr als ein Konditional
\begin{itemize}
	\item {einstelliges Prädikat:}\\
	Das ist ein Randfall, der keine nähere Betrachtung verdient.
	\item mehrstelliges Prädikat:\\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{Bsp}[Konditionale Generalisierung für mehrere Klassen am Beispiel "{}Misanthrope"{}]
				Klasse 1: $ { likes(a,a), likes(b,b), likes(c,c)} $
				Klasse 2: $ {likes(c,b), likes(b,c)} $
				Klasse 3: $ {likes(a,c), likes(a,b), likes(b,a), likes(c,a)} $
			...
			\end{Bsp} 
			\begin{Bsp}[Konditionale Generalisierung für mehrere Klassen am Beispiel "{}ColdSpez"{}]
			Klasse 1: $ { contact(a,a), contact(b,b), contact(c,c)} $
			Klasse 2: $ {contact(c,b), contact(b,c)} $
			Klasse 3: $ {likes(a,c), likes(a,b), likes(b,a), likes(c,a)} $
			Konditional für Klasse 1: $ \{(contact(X,Y))[0.0] <X = Y>\} $\\
			Konditional für Klasse 2: $ \{(contact(X,Y))[0.578] <((X \neq anna \land Y \neq anna) \land X \neq Y)>\} $\\
			Konditional für Klasse 3: $ \{(contact(X,Y))[0.050] <((X = anna \lor Y = anna)\land X \neq Y)>\} $\\
		\end{Bsp} 
