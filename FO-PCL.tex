...
\subsection{Gegenstand und Ansatz}
...
Die Aufgabenstellung dieser Diplomarbeit beinhaltet den Entwurf und die Implementierung einer Inferenzkomponente für FO-PCL unter Verwendung bereits vorhandener Komponenten der Bibilothek Log4KR, eines Projektes der FernUniversität Hagen. Dabei soll die Inferenzkomponente qualitative Konditionale, das sind Konditionale ohne Wahrscheinlichkeit, als Anfrage entgegennehmen und diese mit einer zugeordneten Wahrscheinlichkeit zurückliefern. Es sollen Äquivalenzklassen gebildet werden, die Grundkonditionale mit gleichem Parameterwert zu möglichst allgemeinen Konditionalen zusammenfassen. Dabei wird darauf geachtet werden, so wenig Äquivalenzklassen wie möglich zu bilden, sich gleichzeitig aber nicht in der Ausdrucksfähigkeit der Informationen der Wissensbasis zu beschränken.
...
Zunächst wird in Kapitel \ref{FO-PCL} auf FO-PCL selbst eingegangen und diese ausführlich erläutert. Kapitel \ref{Inferenz} beschreibt den Inferenzprozess in FO-PCL.
Über die einzelnen Phasen der Entwicklung der Inferenzkomponente gibt Kapitel  \ref{Dok} Auskunft.
Die Bedeutung der maximalen Entropie bei der Selektion von Modellen für eine Wissensbasis wird in Kapitel \ref{Alg}  beschrieben. Die Implementation der Inferenzkomponente und der Inferenzprozess \index{Inferenzprozess} und seine Erläuterung anhand von Beispielen ist Inhalt von Kapitel \ref{Inf}. 
Beobachtungen werden in Kapitel \ref{Beob} festgehalten.
...
\subsection{FO-PCL: Syntax und Semantik}
...
\paragraph{Terme und Formeln}\index{Formeln ! Grundformel} \index{Formeln ! logische Formel}
\hspace{10 cm}
\vspace{0.5cm}
\noindent
\\
...
\paragraph{Probabilistische Konditionale}
\hspace{10 cm}
\vspace{0.5cm}
\noindent
\\
...
\begin{Def}[First-Order probabilistisches $ \Sigma $-Konditional]\cite[Kap. 6.2, S. 127, vgl. Def. 6.2.6]{Fis10} \index{Konditional ! FO probab. $ \Sigma $-}
...
\subsubsection{Semantik}\label{sec:semantik}
...
\label{Sol}
\begin{Def}[Grundinstanzen von Konditionalen]\cite[Kap. 6.3, S. 133/134, Def. 6.3.2]{Fis10} \index{Konditionale ! Grundinstanzen von}
...
\end{Def}
\vspace{1cm}
\paragraph{Probabilistische Semantik}
\hspace{10 cm}
\vspace{0.5cm}
...
\subsection{FO-PCL und maximale Entropie} \index{Entropie ! maximale}  \label{Fo-PCL-MaxEnt}
...
\subsubsection{Selektion von Modellen für FO-PCL}\label{Modellselektion}

\paragraph{Selektionskriterium maximale Entropie}
\hspace{10 cm}
\vspace{0.5cm}
\noindent
\\
...
\\
\\
\paragraph{Bedeutung der parametrischen Gemeinsamkeit}  \index{parametrische Gemeinsamkeit}
\label{sec:parÄ}
\hspace{10 cm}
\vspace{0.5cm}
\noindent
...
Voraussetzung für parametrische Gemeinsamkeit ist paarweise parametrische Äquivalenz. Bei der parametrischen Äquivalenz \index{parametrische Äquivalenz} handelt es sich um eine Äquivalenzrelation. Sie liegt vor, wenn zwei Grundinstanzen eines Konditionals \index{Konditional} den gleichen Parameterwert haben. In einer Menge von Grundinstanzen liegt parametrische Gemeinsamkeit dann vor, wenn alle Grundkonditionale paarweise parametrisch äquivalent sind. Ist dies bei allen Grundinstanzen der Fall, so handelt es sich um parametrische Uniformität siehe weiter unten.
...
\\
\\
\paragraph{Bedeutung der parametrischen Uniformität} \index{parametrische Uniformität}
\label{sec:parU}
\hspace{10 cm}
\vspace{0.5cm}
\noindent
...
\section{Inferenz in FO-PCL}\label{Inferenz}
...
\\
\subsection{Inferenz in FO-PCL im Allgemeinen}

\begin{enumerate}
	
\item Wissensbasis:

Das vorhandene Wissen muss in einer Wissensbasis formuliert werden.
...
\subsection{Der Inferenzprozess der Komponente}
...
Das angefragte, unbedingte Konditional wird gemäß der Syntax von FO-PCL grundiert, d.h. es werden alle Grundkonditionale für das angefragte Konditional generiert.
...
\subsubsection{Zusammenfassung der Grundinstanzen des angefragten Konditionals}
...
In dieser Phase werden die gemäß der gleichen Wahrscheinlichkeit zusammengefassten Konditionale einer Klasse zu einem möglichst allgemeinen Konditional zusammengefasst.\\
Die eigentliche Logik der Komponente liegt hierbei in der Erzeugung des jeweiligen Konditionals.
	
\section{Experimentelle Forschungen}
...
\subsection{Einstelliges Prädikat}
\begin{itemize}
...
\subsection{Zweistelliges Prädikat}
\begin{itemize}
...
\begin{itemize}
	...
	\item SportEx \label{BSportEx} s. \ref{SportEx}:\\
...
\subsection{Ein- und zweistellige Prädikate}
\begin{itemize}
...
\subsection{Beobachtungen}
	...
	\begin{itemize}
		...
		\textcolor{blue}{
		\item Unterschiedlich Wahrscheinlichkeiten ergeben sich bei spezifischem Wissen über (eine) bestimmte Konstante des Universums, vgl. \ref{Birds}. 
		\item Unterschiedliche Wahrscheinlichkeiten ergeben sich auch, wenn kein spezifisches Wissen über bestimmte Konstanten in der Wissensbasis, sondern Variablen mit den gleichen Wahrscheinlichkeiten belegt werden. 
		\item Spezifisches Wissen und die Belegung unterschiedlicher Variablen mit gleichen Konstanten stehen in Interaktion
	}
		\item Interaktionen sind nicht trivial und nicht auf den ersten Blick erkennbar TODO Beispiel anführen
		\textcolor{blue}{
		\item Die Anzahl der Sorten scheint auf den ersten Blick keinen Unterschied für die Anzahl der Klassen zu machen; wesentlicher ist das Vorhandensein von spezifischen Konstanten gleich welcher Sorte. Daher wird darauf kein weiterer Fokus gesetzt.} 
	\end{itemize}
...
\section{Betrachtungen}\label{Beob}

\subsection{Vorüberlegungen und erste Ergebnisse}
...
\subsection{Chronologisches Vorgehen}
...
\subsection{Gleichheit von Konditionalen}
Gemäß Aufgabenstellung waren Konditionale, die die gleiche Wahrscheinlichkeit liefern zu einem allgemeinen Konditional zusammenzufassen.\\ 
...
\index{Grundinstanz ! spezifische}
...
\subsubsection{Merkmal der Gleichheit}
...
\subsubsection{Bildung des Constraints}
...
\subsection{Betrachtung des angefragten Konditionals}
...
\textcolor{blue}{
\subsection{Betrachtung der Äquivalenzklassen}
\subsubsection{Vorüberlegungen}
An dieser Stelle sollen die einzelnen Äquivalenzklassen betrachtet werden. Es soll herausgefunden werden, wie die einzelnen Äquivalenzklassen in Relation zueinander aussehen und worin sie sich unterscheiden. Dies dient dazu herauszufinden, ob es eine Systematik gibt, die dazu führen können, einen regelbasierten Algorithmus zu entwickeln. 
\label{Eine Äquivalenzklasse}
\subsubsection{Eine Äquivalenzklasse}
...
\subsubsection{Zwei Äquivalenzklassen}
\begin{itemize}
\item{Zwei Äquivalenzklassen und die Anfrage ist ein Atom}
TODO:
\item {Zwei Äquivalenzklassen und die Anfrage ist eine (nichtatomare, logische) Formel}
TODO:
\item {Zwei Äquivalenzklassen mit je einem Konditional}
TODO:
\item {Zwei Äquivalenzklassen, bei der eine Klasse ein Konditional enthält und die andere Klasse mehr als ein Konditional}
\end{itemize}
\subsubsection{Mehr als zwei Äquivalenzklassen}
\begin{itemize}
\item{Randfälle}\\
\normalfont
\begin{itemize}
\item Alle Grundkonditionale unterscheiden sich von einander in der berechneten Wahrscheinlichkeit:\\
Dies ist ein einfacher Fall und führt zur Ausgabe aller Grundkonditionale mit jeweils berechneter Wahrscheinlichkeit. Dieses Ergebnis wird erzeugt, wenn die Wissensbasis zu jeder Konstanten eine eigene Information bereit hält, die dazu führt, dass sich die Grundkonditionale voneinander unterscheiden. 
\end{itemize}
\item {Allgemein}
\end{itemize}
}
...
Das Ziel des Algorithmus ist die Generierung einer Rückgabemenge von bedingten Konditionalen, die zu dem unbedingten Konditional der Anfrage berechnet wurde. Jedes berechnete Konditional repräsentiert eine Menge von Grundkonditionalen mit gleicher Wahrscheinlichkeit und sollte möglichst allgemein sein. Unter einem allgemeinen Konditional im Sinne dieser Arbeit versteht man ein möglichst kurzes Konditional. Ein Konditional ist möglichst kurz, wenn der logische Constraint eine möglichst einfache Formel ist, also nur so viele atomare Ausdrücke und Junktoren wie unbedingt nötig enthält.\\

\textcolor{blue}{
\begin{Bsp}[Erstrebte Ausgabe am Beispiel "{}Birds"{} \ref{Bsp:Birds}, bzw.  \ref{Birds}]
	\footnote{Unter der angestrebten Ausgabe im Sinne dieser Arbeit ist das Ergebnis eines subjektiven Prozesses der Betrachtung der Klassen der Generierung eines möglichst allgemeinen Konditionals zu verstehen. Das generierte Konditional sollte dabei intersubjektiv als möglichst allgemein akzeptiert werden.}
	Beim Beispiel "{}Birds"{} ergeben sich zwei Klassen, die folgendermaßen aussehen:\\
...
\textcolor{blue}{
	\begin{Bsp}[Erstrebte Ausgabe am Beispiel "{}Misanthrope"{},  \ref{Misanthrope}]
		Beim Beispiel "{}Birds"{} ergeben sich zwei Klassen, die folgendermaßen aussehen:\\
		\\
		Die Ausgabe, die am intuitiv logischsten und möglichst allgemein aussieht und erwünscht wäre, würde so aussehen:
		Konditional für Klasse 1:
		Konditional für Klasse 2: 
	\end{Bsp}
}

\subsection{Idee: Positive Generalisierung}
\subsection{Idee: Negative Generalisierung}
\subsection{Idee: Logische Generalisierungsoptimierung}
...
Die Grundinstanzen, die die gleiche bedingte Wahrscheinlichkeit liefern, sollen dann zu einem möglichst allgemeinen Konditional zusammengefasst werden. Die Rückgabemenge besteht somit aus Äquivalenzklassen, d.h. einer Menge von Konditionalen mit entsprechend gleicher Wahrscheinlichkeit. \\
...
\subsection{Regeln zur Generierung von Constraints, vgl. auch Definition \ref{Def:Constraint} }\label{Reg} 
...
\begin{enumerate}
	...
	\subitem Negativer Constraint bei einstelligen Pädikaten
...
Es wird im Zuge der Generaliserung die Anzahl der Klassen betrachtet. Handelt es sich um eine einzige Klasse, d.h. die dazugehörende Wahrscheinlichkeit ist gleich für alle Grundkonditionale, wird das relationale Konditional mit der berechneten Wahrscheinlichkeit ausgegeben.
...
\section{Implementation der Inferenzkomponente}\label{Inf}
...
\end{figure}
\subsection{Verarbeitung einer Anfrage durch die Komponente}
...
\subsection{Umgebungseinstellungen}


\subsection{Paketstruktur der Komponente}
\begin{itemize}
	\item Inference\\
...
\subsection{Paketstruktur der Tests}
\begin{itemize}
...
\subsection{Klassenstruktur der Komponente}
...
\begin{itemize}
	\item QueryReader
\end{itemize}
\subsection{Klassenstruktur der Tests}
...
\section{Ergebnisse und Ausblick}\label{Erg}
...
\subsection{Probabilistisches Konditional}
\label{probKond}
...
\subsection{Maximale Entropie} \index{Entropie ! maximale}
\label{MaxEnt}
...
\section{Grundlagen der Herbrand-Theorie}
\label{Herb}
...
\section{Modifikation, Korrektur der Bibliothek Log4KR}
\subsection{Korrektur in ConstraintBasedGroundingOperator}
...
\subsection{Korrektur in Fraction}
...
\subsection{Modifikation von EqualityConstraint}
...
\subsection{Modifikation von InequalityConstraint}
...
Anschließend wurde die angestrebte Ausgabe \footnotetext{Unter der angestrebten Ausgabe im Sinne dieser Arbeit ist das Ergebnis eines subjektiven Prozesses der Betrachtung der Klassen der Generierung eines möglichst allgemeinen Konditionals zu verstehen. Das generierte Konditional sollte dabei intersubjektiv als möglichst allgemein akzeptiert werden.} generiert. 
}
\subsection{Birds} Erläuterungen s. \ref{BBirds}
\label{Birds}
\lstinputlisting{Examples/Birds.txt}
\newpage

\subsection{Birds2Birds} Erläuterungen s. \ref{BBirds2Birds}
\label{Birds2Birds}
\lstinputlisting{Examples/Birds2Birds.txt}
\newpage

\subsection{Birds2Classes} Erläuterungen s. \ref{BBirds2Classes}
\label{Birds2Classes}
\lstinputlisting{Examples/Birds2Classes.txt}
\newpage

\subsection{Birds3Classes} Erläuterungen s. \ref{BBirds3Classes}
\label{Birds3Classes}
\lstinputlisting{Examples/Birds3Classes.txt}
\newpage

\subsection{BirdsDiffer} Erläuterungen s. \ref{BBirdsDiffer}
\label{BirdsDiffer}
\lstinputlisting{Examples/BirdsDiffer.txt}
\newpage

\subsection{BirdsEqual} Erläuterungen s. \ref{BBirdsEqual}
\label{BirdsEqual}
\lstinputlisting{Examples/BirdsEqual.txt}
\newpage

\subsection{BirdsNot} Erläuterungen s. \ref{BBirdsNot}
\label{BirdsNot}
\lstinputlisting{Examples/BirdsNot.txt}
\newpage

\subsection{BirdsWithoutNull} Erläuterungen s. \ref{BBirdsWithoutNull}
\label{BirdsWihoutNull}
\lstinputlisting{Examples/BirdsWithoutNull.txt}
\newpage

\subsection{Cold} Erläuterungen s. \ref{BCold}
\label{Cold}
\lstinputlisting{Examples/Cold.txt}
\newpage


	\subsection{ColdSpez} Erläuterungen s. \ref{BColdSpez}
	\label{ColdSpez}
	\lstinputlisting{Examples/ColdSpez.txt}

\newpage

\subsection{ColdTrans} Erläuterungen s.  \ref{BColdTrans}
\label{ColdTrans}
\lstinputlisting{Examples/ColdTrans.txt}

\newpage

	\subsection{ColdTransSpez} Erläuterungen \ref{BColdTransSpez}
	\label{ColdTransSpez}
...
\subsection{Friendship} Erläuterungen s. \ref{BFriendship}
\label{Friendship}
\lstinputlisting{Examples/Friendship.txt}
\newpage

\subsection{FriendshipWithMisanthrope} Erläuterungen s. \ref{BFriendshipWithMisanthrope}
\label{FriendshipWithMisanthrope}
\lstinputlisting{Examples/FriendshipWithMisanthrope.txt}
\newpage

\subsection{Garfield}  Erläuterungen s. \ref{BGarfield}
\label{Garfield}
\lstinputlisting{Examples/Garfield.txt}

\newpage

\subsection{Misanthrope} Erläuterungen s. \ref{BMisanthrope}
\label{Misanthrope}
\lstinputlisting{Examples/Misanthrope.txt}
\newpage

\subsection{MisanthropeIrreflexive} Erläuterungen s. \ref{BMisanthropeIrreflexive}
\label{MisanthropeIrreflexive}
\lstinputlisting{Examples/MisanthropeIrreflexive.txt}
\newpage

\subsection{Monkeys2} Erläuterungen s. \ref{BMonkeys}
\label{Monkeys2}
\lstinputlisting{Examples/Monkeys2.txt}

\newpage

\subsection{MonkeysA} Erläuterungen s. \ref{BMonkeysA}
\label{MonkeysA}
\lstinputlisting{Examples/MonkeysA.txt}

\newpage

\subsection{Sport} Erläuterungen s. \ref{BSport}
\label{Sport}
\lstinputlisting{Examples/Sport.txt}
\newpage

\subsection{SportEx} Erläuterungen s. \ref{BSportEx}
\label{SportEx}
\lstinputlisting{Examples/SportEx.txt}
\newpage


\subsection{SportExN} Erläuterungen s. \ref{BSportExN}
\label{SportExN}
...
\subsection{SportExNFam} Erläuterungen s. \ref{BSportExNFam}
\label{SportExNFam}
...
\newpage
\listoftheorems
\addcontentsline{toc}{section}{Liste der Sätze, Definitionen und Beispiele}
